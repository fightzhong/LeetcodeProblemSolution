...
public class lc_0051_NQueens {
    public List<List<String>> solveNQueens(int n) {
        ...
        solveNQueens( data, 0, 0, n, result );
        return result;
    }

    private void solveNQueens(char[][] data, int x, int y, int remainQueen, List<List<String>> result) {
        if ( x == data.length ) {
            if ( remainQueen == 0 ) {
        ...
        if ( canLocationQueen( data, x, y ) ) {
            data[x][y] = 'Q';
            solveNQueens( data, nextX, nextY, remainQueen - 1, result );
            data[x][y] = '.';
        }

        solveNQueens( data, nextX, nextY, remainQueen, result );
    }
